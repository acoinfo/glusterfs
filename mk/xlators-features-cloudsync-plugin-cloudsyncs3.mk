...
#--------------文件信息--------------------------------------------------------------------------------
#
# 文   件   名: xlators-features-cloudsync-plugin-cloudsyncs3.mk
#
# 创   建   人: RealEvo-IDE
#
...
# Target
#*********************************************************************************************************
LOCAL_TARGET_NAME := cloudsyncs3.so

#*********************************************************************************************************
# Get curl path
#*********************************************************************************************************
LOCAL_CURL_PATH := $(SYLIXOS_BASE_PATH)/libcurl

CHECK_CURL_PATH = $(shell if [ -d $(subst \,/,$(subst $(SPACE),\ ,$(SYLIXOS_BASE_PATH)))/libcurl ]; then echo exist; else echo notexist; fi;)
ifeq ($(CHECK_CURL_PATH), notexist)
LOCAL_CURL_PATH := $(WORKSPACE_libcurl)
endif
...
#*********************************************************************************************************
LOCAL_SRCS :=  \
src/xlators/features/cloudsync/src/cloudsync-plugins/src/cloudsyncs3/src/libcloudsyncs3.c \
src/xlators/features/cloudsync/src/cloudsync-common.c
...
-I"src/rpc/xdr/src" \
-I"src/xlators/features/cloudsync/src/" \
...
-I"$(LOCAL_CURL_PATH)/curl-7.82.0/include"
...
LOCAL_CXXFLAGS :=
LOCAL_LINKFLAGS := -Wl,-rpath="/lib/glusterfs/cloudsync-plugins"
...
-lurcu-cds -lcrypto -lucontext -largp -lgfrpc -lgfxdr -lcurl
