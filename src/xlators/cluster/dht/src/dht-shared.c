...
#ifdef SYLIXOS
        LOCK_DESTROY(&conf->subvolume_lock);
        LOCK_DESTROY(&conf->lock);
        if (conf->defrag) {
            LOCK(&conf->defrag->lock);
            pthread_mutex_destroy(&conf->defrag->dfq_mutex);
            pthread_cond_destroy(&conf->defrag->parallel_migration_cond);
            pthread_cond_destroy(&conf->defrag->rebalance_crawler_alarm);
            pthread_cond_destroy(&conf->defrag->df_wakeup_thread);

            pthread_mutex_destroy(&conf->defrag->fc_mutex);
            pthread_cond_destroy(&conf->defrag->fc_wakeup_cond);
            UNLOCK(&conf->defrag->lock);
            LOCK_DESTROY(&conf->defrag->lock);
            GF_FREE(conf->defrag);
            conf->defrag = NULL;
        }
#endif
