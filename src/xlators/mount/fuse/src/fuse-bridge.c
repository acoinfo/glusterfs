...
#ifdef SYLIXOS
    pthread_mutex_destroy(&fir->handler_mutex);
    pthread_cond_destroy(&fir->handler_cond);
#endif
...
#ifdef SYLIXOS
    /*
     * SylixOS readdir obtains only one directory entry.
     * In order to adapt to libfuse, the operation of pos parameter is processed in glusterfs.
     */
    off_t off = state->off;
#endif
...
#ifdef SYLIXOS
    /* Set max_write as default_page_size */
    fino.max_write = ((struct iobuf_pool *)this->ctx->iobuf_pool)->default_page_size;
#else
    fino.max_write = 1 << 17;
#endif
...
#ifdef SYLIXOS
    /* SylixOS requires a nonexistent directory as a mount point */
    if (access(value_string, F_OK) == 0) {
        gf_log(this_xl->name, GF_LOG_ERROR, "%s %s existed",
...
#ifdef SYLIXOS
                             sync_to_mount ? &ctx->mnt_tid : NULL,
                             &ctx->detached,
#endif
                             priv->status_pipe[1]);
    if (priv->fd == -1)
