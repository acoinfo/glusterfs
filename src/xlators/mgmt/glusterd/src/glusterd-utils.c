...
#ifdef SYLIXOS
        len = sys_readlinkat(fd, dp->d_name, path, sizeof(path) - 1);
#else
...
#ifndef SYLIXOS
    /*
     * trusted.gfid is missing when volume is changed from created to started.
     */
...
#ifdef SYLIXOS
    /*
     * SIGTERM may not working on SylixOS.
     */
    return glusterd_brick_signal(volinfo, brickinfo, options, option_cnt,
                                 op_errstr, SIGKILL);
#else
