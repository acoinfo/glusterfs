...
#ifdef SYLIXOS
static void *daemonize_fork(void *args)
{
    glusterfs_ctx_t *ctx = (glusterfs_ctx_t *)args;
    int err = 1;
    int ret = 0;
    void *retval;

    pthread_setname_np(pthread_self(), "t_gf_daemon");

    if (!ctx->detached) {
        ret = pthread_join(ctx->mnt_tid, &retval);
        if (ret != 0) {
            gf_smsg("daemonize", GF_LOG_ERROR, 0, glusterfsd_msg_43,
                    NULL);
            return NULL;
        }
    ...
    /*
     * SylixOS uses threads instead of fork,
     * so there is no need to keep track of the parent process id.
     */

    /* No one cares about the return value of this thread. */
    return NULL;
}
#endif
...
daemonize(glusterfs_ctx_t *ctx)
{
...
#ifdef SYLIXOS
    ret = pthread_create(NULL, NULL, daemonize_fork, (void *)ctx);
    if (ret != 0) {
    ...
    /* keep child process and make it daemonized */
    daemon(0, 0);
#else
...
#ifdef SYLIXOS
static void set_progname(const char *fullname)
{
    ...
    idx = strrchr(buffer, '/');
    __progname = idx ? strdup(idx + 1) : strdup(buffer);
}
#endif
...
main(int argc, char *argv[])
{
    ...
    int ret = -1;
#ifdef SYLIXOS
    // SylixOS has a PATH_MAX length of 512, which is not sufficient in some cases.
