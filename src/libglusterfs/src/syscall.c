...
#ifdef SYLIXOS
/*
 *  writev() and readv() has different kernel behavior on SylixOS than Linux.
 *  we must combine i/o vector to a single packet.
 */
static struct fuse_iov_wrapper *iov_wrapper_create(struct iovec *iov, int count)
{
    struct fuse_iov_wrapper *wrapper;

    if (!iov || !count) {
        return NULL;
    }

    wrapper = (struct fuse_iov_wrapper *)malloc(sizeof(struct fuse_iov_wrapper));
    if (!wrapper) {
        return NULL;
    }

    wrapper->iov = (struct iovec *)malloc(sizeof(struct iovec) * count);
    if (!wrapper->iov) {
        free(wrapper);
        return NULL;
    }
    ...
    wrapper->count = count;
    memcpy(wrapper->iov, iov, sizeof(struct iovec) * count);

    return wrapper;
}

static void iov_wrapper_destroy(struct fuse_iov_wrapper *wrapper)
{
    if (!wrapper) {
        return;
    }
...
{
#if defined(GF_DARWIN_HOST_OS) || defined(SYLIXOS)
    if (fchdir(dirfd) < 0)
        return -1;
...
#ifdef SYLIXOS
int sys_readlinkat(int dirfd, const char *path, char *buf, size_t bufsize)
{
    if (dirfd != AT_FDCWD && fchdir(dirfd) < 0) {
        return -1;
    }

    return (int)readlink(path, buf, bufsize);
}

static int getfullpath(int dirfd, const char *path, char *fullpath)
{
    int ret = 0;
    char pwd[MAX_FILENAME_LENGTH];

    if (!path || !fullpath) {
        return -1;
    }

    memset(pwd, 0, MAX_FILENAME_LENGTH);
    memset(fullpath, 0, MAX_FILENAME_LENGTH);

    if (dirfd == AT_FDCWD || path[0] == '/') {
        strncpy(fullpath, path, MAX_FILENAME_LENGTH - 1);
        return 0;
    }
    ...
    if (!getcwd(pwd, MAX_FILENAME_LENGTH)) {
        return -1;
    }

    if (strlen(pwd) + strlen(path) > MAX_FILENAME_LENGTH - 1) {
        return -1;
    }

    snprintf(fullpath, MAX_FILENAME_LENGTH - 1, "%s/%s", pwd, path);
...
{
#ifdef SYLIXOS
    char fulloldpath[MAX_FILENAME_LENGTH];
    char fullnewpath[MAX_FILENAME_LENGTH];

    if (getfullpath(oldfd, oldpath, fulloldpath) ||
        getfullpath(newfd, newpath, fullnewpath)) {
        return -1;
    }

    return link(fulloldpath, fullnewpath);
#else
...
#ifdef SYLIXOS
    /*
     *  SylixOS dose not support lchown.
     */
    return FS_RET_CHECK0(chown(path, owner, group), errno);
#else
...
sys_readv(int fd, const struct iovec *iov, int iovcnt)
{
    ...
    buffer = GF_MALLOC(total, gf_common_mt_iovec);
    if (!buffer) {
        gf_log("glusterfs", GF_LOG_ERROR, "Out of memory");
        return -1;
    }

    ret = read(fd, buffer, total);
    if (ret < 0) {
        GF_FREE(buffer);
        return -1;
    }
    ...
    pos = buffer;
    for (int i = 0; i < iovcnt; ++i) {
        if ((ssize_t)(recieve - iov[i].iov_len) >= 0) {
            move = iov[i].iov_len;
            recieve -= iov[i].iov_len;
        } else {
            move = recieve;
            recieve = 0;
        }
        memcpy(iov[i].iov_base, pos, move);
        pos += move;
    }
...
#ifdef SYLIXOS
    /*
     * For SylixOS fuse device, use fuse_iov_wrapper.
     */
    if (g_fuse_fd == fd) {
        struct fuse_iov_wrapper *wrapper = NULL;
        int ret = 0;
        struct fuse_out_header *fouh = NULL;

        wrapper = iov_wrapper_create(iov, iovcnt);
        if (!wrapper) {
            return -ENOMEM;
        }

        ret = write(fd, wrapper, sizeof(struct fuse_iov_wrapper));
        ...
        iov_wrapper_destroy(wrapper);

        return (ret < 0) ? -1 : ret;
    } else {
        /*
         * Use encapsulated writev, otherwise the content sent by rpc cannot be parsed in wireshark.
         */
        int total = 0;
        int ret = 0;
        void *buffer = NULL;
...
{
#if defined(GF_LINUX_HOST_OS) || defined(__NetBSD__) || defined(SYLIXOS)
