...
#ifdef SYLIXOS
    /*
     * Using event array is not necessary here, We set this for bypass a bug[libsylixos-issues-178].
     */
...
#ifdef SYLIXOS
        /*
         * glusterd create one epoll thread, and it will blocking/deadlock when peer probe remotes.
         * We have to set timeout for this situation.
         */
#define EPOLL_TIMEOUT 3000

        int timeout = -1;
        if (event_pool->eventthreadcount == 1) {
            timeout = EPOLL_TIMEOUT;
        }

        cnt = epoll_wait(event_pool->fd, event, MAX_EVENTS, timeout);
        ...
        for (int i = 0; i < cnt; ++i) {
            ret = event_dispatch_epoll_handler(event_pool, &event[i]);
            if (ret) {
