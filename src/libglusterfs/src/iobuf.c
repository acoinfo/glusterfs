...
static const struct iobuf_init_config gf_iobuf_init_config[] = {
    /* { pagesize, num_pages }, */
    {128, 128},     {512, 64},       {2 * 1024, 64}, {8 * 1024, 32},
    {32 * 1024, 32}, {128 * 1024, 16}, {256 * 1024, 8}, {1 * 1024 * 1024, 2},
};
#else
static const struct iobuf_init_config gf_iobuf_init_config[] = {
    /* { pagesize, num_pages }, */
...
#ifndef SYLIXOS
    /*
     * iobuf_get_from_small got mutex resource leak so we don't want to use it.
     */
...
#ifdef SYLIXOS
        /*
         * Maybe we should release mutex here which created by iobuf_get_from_small.
         */
        LOCK_DESTROY(&iobuf->lock);
#endif
        GF_FREE(iobuf->free_ptr);
...
#ifdef SYLIXOS
    /*
     * Fix iobref_new mutex resource leak
     */
