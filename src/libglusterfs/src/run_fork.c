...
runner_start(runner_t *runner)
{
    ...
    for (i = 0; i < 3; i++) {
        ret |= posix_spawn_file_actions_addclose(&file_actions, pi[i][i ? 0 : 1]);
    }
    ...
    for (i = 0; i < 3; i++) {
        switch (runner->chfd[i]) {
            ...
            case -2:
                /* redir to pipe */
                ret |= posix_spawn_file_actions_adddup2(&file_actions, pi[i][i ? 1 : 0], i);
                break;
            default:
                /* redir to file */
                ret |= posix_spawn_file_actions_adddup2(&file_actions, runner->chfd[i], i);
        }
    }
    ...
    ret = posix_spawnp(&runner->chpid, runner->argv[0], &file_actions, NULL, runner->argv, NULL);
    if (ret) {
        goto close_file;
    }
