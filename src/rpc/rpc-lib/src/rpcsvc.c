...
rpcsvc_program_destroy(rpcsvc_program_t *program)
{
    if (program) {
#ifdef SYLIXOS
        if (program->req_queue_key) {
            pthread_key_delete(program->req_queue_key);
        }
        for (int i = 0; i < EVENT_MAX_THREADS; i++) {
            if (program->request_queue[i].queue_lock.PMUTEX_ulMutex) {
                pthread_mutex_destroy(&program->request_queue[i].queue_lock);
                pthread_cond_destroy(&program->request_queue[i].queue_cond);
            }
        }
#endif
