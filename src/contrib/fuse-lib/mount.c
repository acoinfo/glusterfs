...
static void mount_args_free(mount_args_t **args)
{
    if (!args && !*args) {
        return;
    }
...
static void *mount_fork(void *args)
{
    /* hello it's mount agent */
    mount_args_t *mount_args = (mount_args_t *)args;
    int ret = 0;

    pthread_setname_np(pthread_self(), "t_gf_mount_agent");

    ret = fuse_mount_sys (mount_args->mountpoint,
                          mount_args->fsname,
                          mount_args->mnt_param,
                          mount_args->fd);
    if (ret == -1) {
...
#if 0
            /* We don't use fusermount on SylixOS */
            ret = fuse_mount_fusermount (mount_args->mountpoint,
                                         mount_args->fsname,
                                         mount_args->mnt_param,
                                         mount_args->fd);
#endif
    ...
    if (mount_args->status_fd >= 0)
        (void)write (mount_args->status_fd, &ret, sizeof (ret));

    mount_args_free(&mount_args);
               ...
               int status_fd)
{
        ...
        mount_args = (mount_args_t *)malloc(sizeof(mount_args_t));
        if (!mount_args) {
            GFFUSE_LOGERR ("malloc failed (%s)", strerror (errno));
            return -1;
        }

        mount_args->mountpoint = strdup(mountpoint);
        mount_args->fsname = strdup(fsname);
        mount_args->mnt_param = strdup(mnt_param);
        mount_args->status_fd = status_fd;
        mount_args->fd = fd;

        ret = pthread_create(&tid, NULL, mount_fork, (void *)mount_args);
        if (ret != 0) {
            close (fd);
            fd = -1;
            return fd;
